...
# 🦁 XXD Panel 066｜천진한 서사의 선

### 편안한 검정색 선과 부드러운 평면 페인트를 사용하여 어린이답고 절제된 사진 스토리를 전달합니다
...
가족과 반려동물, 여행, 일상의 교감을 담은 사진은 편안한 일러스트로 옮기기 좋습니다. 하지만 흔한 만화 템플릿은 인물 관계를 획일적인 표정으로 바꾸거나 지나치게 많은 귀여운 요소로 화면을 채우곤 합니다. 그러면 ‘동심’은 남아도 원본 사진 속 동작과 이야기는 사라집니다.

**Panel 066**은 가장 기억할 만한 대상의 관계를 보존하고, 조금 서툰 듯한 검정 선과 3~6가지 부드러운 평면 색으로 장면을 다시 들려줍니다. 흔들림과 멈춤, 수정 흔적은 허용하되 무게중심과 리듬, 여백을 성숙하게 조절해 천진함이 유치함이 되지 않게 합니다.

### 이런 상황에 적합합니다

- **가족, 반려동물, 여행 또는 일상의 교감을 기록하고 싶을 때:** 완전한 배경보다 대상의 동작과 관계가 더 중요합니다.
- **동심은 좋지만 틀에 박힌 귀여움은 싫을 때:** 단순화한 비례와 느슨한 검정 선이 실제 관찰을 보존하고 모두를 똑같은 만화 표정으로 만들지 않습니다.
- **편안하면서도 디자인 판단이 느껴지는 화면을 원할 때:** 원본에서 가져온 3~6가지 부드러운 색, 중심에서 비껴난 대상과 넉넉한 여백이 함께 성숙한 리듬을 만듭니다.
...
> XXD Panel 066으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
**키워드:** 어린아이 같은 서사 · 어설픈 검은 선 · 부드러운 평면화 · 힐링 컬러 · 손글씨 관찰

Panel 066는 약간 어색한 검정색 선, 3~6개의 파스텔 색상 블록 및 손으로 쓴 관찰 문장을 사용하여 사진에서 가장 기억에 남는 주제 관계와 몇 가지 환경적 단서를 유지하여 전문적으로 편집된 자유형 그림을 만듭니다.

<!-- xxd-panel-benefit:start -->
## 빠른 판단: XXD Panel 066이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 편안한 검정색 선과 부드러운 평면 페인트를 사용하여 어린이답고 절제된 사진 스토리를 전달합니다 |
| **한눈에 알아볼 특징** | 어린아이 같은 서사 · 어설픈 검은 선 · 부드러운 평면화 · 힐링 컬러 · 손글씨 관찰 |
...
기억할 가치가 있는 행동, 관계, 이야기를 순진하지만 알아볼 수 있는 검은 그림과 부드럽고 평면적인 색상으로 압축합니다. 캐주얼한 스케치의 느슨함과 수정 표시는 그대로 유지되며, 전문적인 편집 판단으로 무게, 리듬, 공간을 제어합니다.

```text
기본 동작 및 관계 찾기 → 전체 장면 및 까다로운 세부 사항 제거 → 단순화된 비율 및 어색한 검은 선으로 다시 그리기 → 소스에서 3~6개의 부드러운 평면 색상 변환 → 오프셋된 피사체와 넉넉한 공간을 통해 리듬 구축 → 자연스러운 관찰 쓰기
```

## 완성 스타일을 알아보는 특징

- 주요 주제와 관계는 즉시 알아볼 수 있으며 건물, 식물, 사람 및 사물은 의도적으로 단순화됩니다.
- 검정색 선은 미세하고 직접적이며 흔들림, 일시 중지, 끊김 및 일상적인 수정이 포함되어 불완전하게 안정적입니다.
- 3~6개의 원본 색상은 반투명하고 약간 채도가 낮은 플랫 블록이 되며 몇 가지 명확한 액센트만 있습니다.
- 그림은 필드를 채우지 않습니다. 주제는 액션에 따라 이동, 축소 또는 전개되는 반면 보조 요소는 스토리를 뒷받침할 뿐입니다.
- 작은 글씨체는 나중에 붙인 광고 제목이 아니라, 그린 후 자연스럽게 덧붙여진 하나의 관찰 같은 느낌이다.

미적 제약과 금지 항목은 [중국어 원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 런타임 어댑터는 이번 전달 변수만 처리합니다. [Skill 워크플로](SKILL.md) · [영문 런타임 어댑터](references/xxd-panel-066-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 066의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-066 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-066 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-066 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-066 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-066 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-066 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 066 적용, 공통 설정은 한 번만 확정
/xxd-panel-066 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-066`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-066-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-066-prompt.zh-CN.md)
...
| **[xxd-panel-066](https://github.com/nevertoday/xxd-panel-066)** | 검은 주도형 · 거대한 여백 · 망점 소멸 · 선적 사유 · 생각의 파편 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–066) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
