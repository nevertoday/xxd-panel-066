...
**Panel 066** 适合这类任务：需要保留原图核心内容，同时希望最终视觉达到以下方向：**用松弛黑线和柔和平涂讲述童真而克制的照片故事**。它先从用途与期望结果出发，再决定构图、尺寸、文字和具体输出形式。
...
把照片里真正值得记住的动作、关系和故事压缩成童真但可辨的黑线与柔和平涂色块。它保留随手画的松弛与修正痕迹，同时用专业编辑判断控制重心、节奏和空白。

```text
找出主要动作与关系 → 删除完整场景和繁琐细节 → 用概括比例与笨拙黑线重画 → 从原图转译 3–6 种柔和平涂色 → 以偏置主体和大留白建立节奏 → 写下一句自然观察
```

## 成品中最容易识别的风格特点

- 关键主体与关系一眼可辨，但建筑、植物、人物和器物都被主动概括。
- 黑线纤细、直接、不完全稳定，带抖动、停顿、断开与随手修正。
- 3–6 种源图颜色转化为清透、略去饱和的平涂块，并保留少量鲜明跳点。
- 插画不填满画面；主体偏置、缩小或沿动作方向展开，次要元素只服务叙事。
- 小型手写文字像画完后自然记下的一句话，而不是后贴的商业标题。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-066-prompt.en.md)
...
| **[xxd-panel-066](https://github.com/nevertoday/xxd-panel-066)** | 黑色主形 · 极大留白 · 网点消散 · 禅意思考 · 哲思碎片 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–066） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
