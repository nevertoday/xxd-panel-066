...
# 🦁 XXD Panel 066｜無邪気な物語の線

### リラックスした黒の線と柔らかいフラット ペイントを使用して、子供らしく抑制された写真のストーリーを伝えます
...
家族、ペット、旅、日常のやり取りは、軽やかなイラストにしやすい題材です。しかし、よくあるカートゥーンのテンプレートは人物同士の関係を画一的な表情に置き換えたり、かわいい要素で画面を埋めすぎたりします。その結果、「童心」はあっても、元写真にあった動きや物語が失われてしまいます。

**Panel 066** は、最も記憶に残したい主体同士の関係を保ち、少し不器用な黒線と 3〜6 色の柔らかなフラットカラーで場面を語り直します。線の揺れ、ためらい、描き直しを許しながら、重心、リズム、余白を大人の判断で整えることで、無邪気さを幼稚さに変えません。

### このような場面に適しています

- **家族、ペット、旅、日常のやり取りを残したい：** 背景を完全に描くことより、主体の動きと関係が大切です。
- **童心は欲しいが、型通りのかわいさは避けたい：** 大づかみな比率と力の抜けた黒線が実際の観察を保ち、皆を同じカートゥーン表情にしません。
- **気楽さとデザイン上の判断を両立したい：** 原画像から取る 3〜6 色の柔らかな色、中心をずらした主体、大きな余白が一緒に成熟したリズムを作ります。
...
> XXD Panel 066 でこの画像を処理して、まず最適な構図とサイズを提案して。
...
**キーワード:** 子供っぽい物語 · ぎこちない黒い線 · 柔らかな平面絵 · 癒しの色彩 · 手書きの観察

Panel 066 は、最も記憶に残る被写体の関係性と環境に関するいくつかの手がかりを写真に残し、少しぎこちない黒い線、3 ～ 6 個のパステル カラー ブロック、手書きの観察文を使用して、プロが編集したフリーハンドの図面を作成します。

<!-- xxd-panel-benefit:start -->
## すぐに判断：XXD Panel 066 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | リラックスした黒の線と柔らかいフラット ペイントを使用して、子供らしく抑制された写真のストーリーを伝えます |
| **ひと目で分かる特徴** | 子供っぽい物語 · ぎこちない黒い線 · 柔らかな平面絵 · 癒しの色彩 · 手書きの観察 |
...
覚えておく価値のあるアクション、人間関係、ストーリーを、素朴だが認識可能な黒い描画と柔らかいフラットな色に圧縮しています。カジュアルなスケッチの緩さや修正跡はそのままに、プロの編集判断によって重み、リズム、スペースがコントロールされます。

```text
主要なアクションと関係性を見つける → 完全なシーンと煩雑なディテールを削除する → 単純化されたプロポーションとぎこちない黒い線で再描画する → ソースから 3 ～ 6 色の柔らかい単色を変換する → オフセットされた主題と十分なスペースを通じてリズムを構築する → 自然な観察を 1 つ書く
```

## 完成作品を見分けるポイント

- 建物、植物、人物、物体は意図的に単純化されていますが、主要な主題と関係は即座に認識できるままです。
- 黒の線は細く、直接的ですが、安定性が不完全で、ぐらつき、一時停止、中断、および偶然の修正が見られます。
- 3 ～ 6 色のソース カラーが半透明の、わずかに彩度の低いフラット ブロックになり、いくつかの明確なアクセントのみが追加されます。
- イラストがフィールドを埋めることはありません。主題はアクションに沿って移動、縮小、または展開しますが、二次的な要素はストーリーをサポートするだけです。
- 小さな手書きの文字は、商業的なタイトルを後から貼り付けたものではなく、描いた後に自然に追加された 1 つの観察のように感じられます。

美的制約と避けるべき表現は[中国語の原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは今回の納品条件だけを扱います。 [Skill ワークフロー](SKILL.md) · [英語ランタイムアダプター](references/xxd-panel-066-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 066 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-066 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-066 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-066 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-066 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 066 を適用し、共通設定は一度だけ解決
/xxd-panel-066 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-066`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-066-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-066-prompt.zh-CN.md)
...
| **[xxd-panel-066](https://github.com/nevertoday/xxd-panel-066)** | 黒い主形 · 巨大な余白 · 網点の消散 · 禅的思考 · 思考の断片 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–066） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
